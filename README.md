...
当前实现包含 v0.1 协议基线、两个原生 Hook Adapter、确定性监督器和 SQLite 持久化运行时：
...
冷进程性能基准和跨平台契约测试。尚未实现 LLM Judge、OpenTelemetry exporter 和一键 Hook 安装。
daemon 已从正确性前置条件降级为有性能证据后才启用的可选部署方式。
