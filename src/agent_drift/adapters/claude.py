"""Claude Code native lifecycle Hook adapter."""
...
class ClaudeCodeAdapter(PlatformAdapter):
    """Adapter for the current Claude Code command-hook JSON contract."""
    ...
    def capabilities(self) -> PlatformCapabilities:
        return PlatformCapabilities(
            ...
            notes=(
                "PostCompact and SessionEnd are observation-only lifecycle points.",
                "Stop continuation is capped by Claude Code after repeated blocks.",
            ),
        )
    ...
    def adapt_event(
        ...
        *,
        ...
        return build_event(
            platform="claude-code",
            platform_version=self._platform_version,
            extension_namespace="claude-code",
            allowed_hooks=_CLAUDE_HOOKS,
            raw_input=raw,
            timestamp=timestamp,
            repo_root=repo_root,
            sequence=sequence,
    ...
    def render_decision(self, event: AgentEvent, decision: GuardDecision) -> HookResponse:
        return render_with_fallback(
            ...
            lambda current, guard_decision, action: common_hook_response(
                ...
                guard_decision,
                action,
                precompact_uses_continue=False,
            ),
        )
